...
// --- MODIFIED: New data and colors for the 6-gas pie chart ---
const pieData = [
    { name: 'CO₂', value: 74 },
    { name: 'CH₄', value: 15 },
    { name: 'NOx', value: 5 },
...
const COLORS = ['#3b82f6', '#8b5cf6', '#ef4444', '#f97316', '#10b981', '#eab308']; // Blue, Purple, Red, Orange, Green, Yellow

// Other data sets (no changes needed)
            ...
            <div className="relative z-10">
                ...
                <motion.main className="px-8 pb-8" initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.15, delayChildren: 0.2 } } }}>
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                        ...
                        <div className="lg:col-span-1 flex flex-col gap-8">
                            <GlassCard className="p-6 text-center h-[450px]">
                                {/* MODIFIED Title */}
                                <motion.h3 variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } } }} className="text-lg font-semibold text-slate-300 mb-4">Overall Emission Composition</motion.h3>
                                <ResponsiveContainer width="100%" height="100%">
                                    {/* MODIFIED PieChart to use new data */}
                                    <PieChart margin={{ top: 40, right: 40, bottom: 40, left: 40 }}>
                                        <Pie data={pieData} cx="50%" cy="50%" labelLine={false} label={<AnimatedCustomLabel />} innerRadius={80} outerRadius={100} dataKey="value" paddingAngle={5}>
                        ...
                        <motion.div variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.15, delayChildren: 0.2 } } }} className="lg:col-span-2 grid grid-cols-1 gap-8">
                            ...
                            <GlassCard className="p-6">
                                <motion.h3 variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } } }} className="flex items-center gap-2 text-lg font-semibold text-slate-300 mb-4"><BarChartHorizontal size={20} /> Particulate Matter Trend (MtCO₂e)</motion.h3>
                                <ResponsiveContainer width="100%" height={250}>
                                    <AreaChart data={particulateMatterData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                                        <defs>
                                            ...
                                            <linearGradient id="color_pm25" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#eab308" stopOpacity={0.4}/><stop offset="95%" stopColor="#eab308" stopOpacity={0}/></linearGradient>
                                        </defs>
                                        ...
                                        <Area type="monotone" dataKey="PM2.5" name="PM2.5" stroke="#eab308" strokeWidth={3} fill="url(#color_pm25)" />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </GlassCard>
                        </motion.div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                         ...
                         <GlassCard className="lg:col-span-1 p-6"> <motion.h3 variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } } }} className="text-lg font-semibold text-slate-300 mb-6">Mitigation Project Effectiveness</motion.h3> <div className="space-y-6"> {mitigationData.map((project) => ( <motion.div key={project.name} variants={{ hidden: { opacity: 0, y: 10 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } } }}> <div className="flex items-center gap-3 mb-2"> <project.icon className={`w-6 h-6 ${project.color}`} /> <span className="font-semibold">{project.name}</span> <span className="ml-auto text-slate-300">{project.effectiveness}%</span> </div> <div className="w-full bg-slate-700/50 rounded-full h-2.5"> <motion.div className="bg-gradient-to-r from-blue-500 to-purple-500 h-2.5 rounded-full" initial={{ width: 0 }} whileInView={{ width: `${project.effectiveness}%` }} viewport={{ once: true }} transition={{ duration: 1, ease: 'easeOut', delay: 0.2 }}></motion.div> </div> </motion.div> ))} </div> </GlassCard>
